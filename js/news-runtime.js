...
(function(){
  ...
  const endpoint=cfg.SUPABASE_URL.replace(/\/$/,'')+'/rest/v1/news';
  ...
  async function load(initial){if(busy)return;busy=true;if(initial){offset=0;box.innerHTML='<div class="article-item"><h2>बातम्या लोड होत आहेत...</h2></div>'}else{const b=document.querySelector('#newsLoadMore button');if(b)b.disabled=true}
    ...
    try{
      ...
      const r=await fetch(endpoint+'?'+params,{headers,signal:ctl.signal});clearTimeout(timer);
      if(!r.ok)throw new Error('HTTP '+r.status);
      const rows=await r.json();
      if(initial)box.innerHTML='';
      if(!rows.length&&!offset){box.innerHTML='<div class="article-item"><h2>सध्या नवीन बातम्या उपलब्ध नाहीत.</h2><p>नवीन बातमी पाठवा — Admin मंजुरीनंतर ती येथे दिसेल.</p></div>'}
      else if(rows.length)box.insertAdjacentHTML('beforeend',rows.map(card).join(''));
      offset+=rows.length;more=rows.length===limit;bind();controls();
    }catch(err){console.warn('News load:',err);if(initial)box.innerHTML='<div class="article-item"><h2>बातम्या सध्या लोड होत नाहीत.</h2><p>कृपया पुन्हा प्रयत्न करा.</p><button class="feed-primary" type="button" id="newsRetry">↻ पुन्हा प्रयत्न</button></div>';else{const b=document.querySelector('#newsLoadMore button');if(b)b.disabled=false}}
    finally{busy=false}
  }
  load(true);
})();
