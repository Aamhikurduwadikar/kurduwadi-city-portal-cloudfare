/* Homepage: load exactly 2 approved Kurduwadi pride personalities. */
(function(){
  'use strict';
  ...
  async function run(){
    ...
    if(!host)return;
    host.style.display='grid';host.style.visibility='visible';
    ...
    try{
      ...
      const r=await fetch(cfg.SUPABASE_URL.replace(/\/$/,'')+'/rest/v1/personalities?'+params,{headers:{apikey:cfg.SUPABASE_ANON_KEY,Authorization:'Bearer '+cfg.SUPABASE_ANON_KEY},cache:'no-store'});
      ...
      host.innerHTML=rows.map(p=>{const photo=mediaUrl(p.photo_url),desc=p.major_work||p.achievements||p.social_contribution||'कुर्डुवाडीचे प्रेरणादायी व्यक्तिमत्त्व.';return `<a class="pride-card home-pride-card" href="personality-view.html?id=${encodeURIComponent(p.id)}"><div class="pride-card-photo">${photo?`<img src="${esc(photo)}" alt="${esc(p.name)}" loading="lazy" decoding="async">`:'<span>🏅</span>'}</div><div class="pride-card-body"><span class="pride-card-label">KURDUWADI PRIDE</span><b>${esc(p.name)}</b><small>${esc(p.field||p.position||'अभिमानास्पद व्यक्तिमत्त्व')}${p.position&&p.field?' • '+esc(p.position):''}</small><p>${esc(String(desc).replace(/<[^>]*>/g,'').slice(0,105))}</p><strong class="pride-card-link">Profile पहा →</strong></div></a>`}).join('');
  ...
  window.addEventListener('kurduwadi:restored',run);
})();
